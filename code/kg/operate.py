...
async def kg_query(
    ...
    # 抽取问题中的关键词
    # "low_level_keywords"：来源于‘对话历史’，侧重于对话历史中用户提及的症状实体 ll_keywords
    # "high_level_keywords"：来源于‘当前查询’，侧重于查询文本中与疾病相关的实体 hl_keywords
    ...
    dict_data_med = {}
    for dm in data_med:
        dict_data_med[dm] = context.count(dm)
    filtered_diseases = {k: v for k, v in dict_data_med.items() if v > len(ll_keywords)//2}
    data_med = sorted(filtered_diseases.keys(), key=lambda x: filtered_diseases[x], reverse=True)
    

    knowledge_context= PROMPTS["knowledge"]
    knowledge_context= knowledge_context.format(
        knowledge=context,
    )
    extract_knowledge = PROMPTS["list_all_diseases"]
    extract_knowledge= extract_knowledge.format(
        keywords=ll_keywords_str,
    )

    extract_context = await use_model_func(
            prompt = extract_knowledge,
            system_prompt=knowledge_context,
            stream=query_param.stream,
        )

    extract_context = re.sub(r'<think>.*?</think>', '', extract_context, flags=re.DOTALL)
    pattern = r"\[.*?\]"
    match = re.search(pattern, extract_context, re.DOTALL)
    if match:
        list_str = match.group().replace("\n","").replace(" ","")  # 获取匹配到的列表字符串
        try:
            extract_context = ast.literal_eval(list_str)
    ...
    history_context = "\n".join(query.split("\n")[2:])
    #系统提示
    sys_prompt_temp = system_prompt if system_prompt else PROMPTS["low_level_keywords_system"] 
    sys_prompt_temp = sys_prompt_temp.format(
        context_data=extract_context
    )
    context1 = sys_prompt_temp
    #用户输入
    prompt_temp = PROMPTS["low_level_keywords_prompt"]
    prompt_temp = prompt_temp.format(
        history=history_context,
        keywords = ll_keywords_str,
    )
    context2 = prompt_temp
    if query_param.only_need_prompt:
        return prompt_temp
        
    len_of_prompts = len(encode_string_by_tiktoken(sys_prompt_temp + prompt_temp))
    ...
    if match:
        list_str = match.group().replace("\n","").replace(" ","")  # 获取匹配到的列表字符串
        try:
            response = ast.literal_eval(list_str)
        except (ValueError, SyntaxError) as e:
            print(f"转换出错: {e}")
    else:
        print("未找到列表")
    ...
    response_str = ", ".join(response) if response else ""
    response = ll_keywords_str + '11111' + hl_keywords_str + '11111' + response_str #高级关键词+可能性疾病
    
    return response


# async def kg_query_context(
#     query: str,
#     hl_keywords: list,
#     ills: list,
#     knowledge_graph_inst: BaseGraphStorage,
...
#         knowledge=context,
#     )
#     extract_knowledge = PROMPTS["extract_knowledge_ill"]
#     extract_knowledge= extract_knowledge.format(
#         data=hl_keywords_str,
#         keywords=response_str,
#         # knowledge=context,
#     )
...
#             stream=query_param.stream,
#         )
#     extract_context = re.sub(r'<think>.*?</think>', '', extract_context, flags=re.DOTALL).strip()

#     return extract_context

async def kg_query_result(
    query: str,
    ll_keywords: list,
    hl_keywords: list,
    ills: list,
    ...
    #系统提示
    sys_prompt_temp = system_prompt if system_prompt else PROMPTS["high_level_keywords_system"]
    sys_prompt_temp = sys_prompt_temp.format(
        content_data=extract_context,
    ...
    #用户输入
    prompt_temp = PROMPTS["high_level_keywords_prompt"]
    prompt_temp = prompt_temp.format(
        history=history_context,
        keywords=ll_keywords_str,
        ills=response_str,
    ...
    if isinstance(response, str) and len(response) > len(sys_prompt_temp):
        response = (
            response.replace(sys_prompt_temp, "")
            .replace("user", "")
            .replace("model", "")
            .replace(query, "")
            .replace("<system>", "")
...
async def _build_query_context(
    ...
    elif query_param.mode == "ll_medical":
        if relate_datas==['疾病','症状']:
            entities_context, relations_context, text_units_context = await _get_node_data(
                ll_keywords,
                relate_datas,
                knowledge_graph_inst,
...
async def _get_node_data(
    ...
    logger.info(
        f"Query nodes: {query_str}, top_k: {query_param.top_k}, cosine: {entities_vdb.cosine_better_than_threshold}"
    )

    if query:
        top_k = query_param.top_k//len(query) + 1
        results = []
        for mn in query:
            aresult = await entities_vdb.query(
                mn, top_k=top_k, ids=query_param.ids
            )
        ...
        for mn in results:
            # print(mn)
            if mn not in aresults:
                aresults.append(mn)
        results = aresults
    else:
        #关键词进行实体匹配，返回vdb
        results = await entities_vdb.query(
            query_str, top_k=query_param.top_k, ids=query_param.ids
        )
    ...
    #去除无关节点
    if label:
        pnode_datas, pnode_degrees =  [],[]
        for i,adata in enumerate(node_datas):
            if any(adata['entity_type'] in item for item in relate_datas):
                pnode_datas.append(adata)
                pnode_degrees.append(node_degrees[i])
        
        node_datas = pnode_datas
        node_degrees = pnode_degrees
    ...
    use_text_units, use_relations = await asyncio.gather(
        ...
        _find_most_related_edges_from_entities(
            node_datas, relate_datas, query_param, knowledge_graph_inst
        ),
    )

    len_node_datas = len(node_datas) #节点的数量
    ...
    entities_context = list_of_list_to_csv(entites_section_list)#返回一个符合 CSV 格式的纯文本字符串
...
async def _find_most_related_edges_from_entities(
    ...
    ##all_edges_pack边的详细信息，all_edges_degree边的权重
    ...
    for k, v, d in zip(all_edges, all_edges_pack, all_edges_degree):
        if v is not None:
            for r in relate_datas:
                if r in v['keywords']:
                    all_edg = {"src_tgt": k, "rank": d, **v}
                    all_edges_data.append(all_edg)
                    break
    # all_edges_data = [
    #     {"src_tgt": k, "rank": d, **v}
    #     for k, v, d in zip(all_edges, all_edges_pack, all_edges_degree)
